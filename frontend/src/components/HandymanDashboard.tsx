...
import { completeInitialVisit } from "../lib/quoteEscrowApi";
import {
  Clock,
  Briefcase,
  CheckCircle,
...
const DASHBOARD_FILTERS: {
  ...
  { id: "all", label: "All" },
  {
    ...
    badgeClass: "bg-[#EF9D39] text-black",
  },
  {
    id: "active",
...
export default function HandymanDashboard() {
  ...
  const [isCalendarOpenFor, setIsCalendarOpenFor] = useState<number | null>(
    null,
  );
  const [counterValues, setCounterValues] = useState<
    Record<
      number,
      { proposedTime: Date | null; message: string; duration: number }
    >
  >({});
  ...
  const handleCounterJob = async (jobId: number) => {
    ...
    if (!value?.duration || value.duration <= 0) {
      setActionError(
        "Please specify the estimated duration for the counter offer.",
      );
      return;
    }
    ...
    if (!value?.message || value.message.trim() === "") {
      setActionError(
        "Please include a message for the client with your counter offer.",
      );
      return;
    }
  ...
  const handleCompleteVisit = async (jobId: number) => {
    try {
      setActionError("");
      setActionLoadingId(jobId);
      await completeInitialVisit(jobId);
      await fetchJobs();
    } catch (error: any) {
      setActionError(
        error.response?.data?.error || "Failed to complete visit.",
      );
    } finally {
      setActionLoadingId(null);
    }
  ...
  const filterCounts = DASHBOARD_FILTERS.reduce(
    (acc, filter) => {
      if (filter.id === "all") {
        ...
        acc[filter.id] = jobs.filter(
          (j) => getJobCategory(j) === filter.id,
        ).length;
      }
      return acc;
    },
    ...
    <div className="space-y-8 mt-10 p-4 max-w-6xl mx-auto">
      ...
      {showAvailable && (
        <div>
          ...
          {actionError && (
            <p className="mb-4 text-sm font-black uppercase text-red-500">
              {actionError}
            </p>
          )}
                        ...
                        {awaitingClient ? (
                          ...
                          job.client_proposed_time && (
                            <p className="text-xs font-black uppercase text-blue-500">
                              Client requested:{" "}
                              {formatDateTime(job.client_proposed_time)}
                            </p>
                          )
                        )}
                      </div>
      ...
      {showActive && (
        <div>
          <h2 className="text-2xl font-black uppercase mb-4 flex items-center gap-2 text-black dark:text-white">
            <Briefcase className="text-blue-500" strokeWidth={3} /> My Active
            Jobs
          </h2>
          <div className="grid gap-4">
            {acceptedJobs.length > 0 ? (
              acceptedJobs.map((job) => (
                <div
                  key={job.id}
                  className="bg-white dark:bg-zinc-800 border-[3px] border-blue-500 p-5 rounded-2xl flex justify-between items-center shadow-[5px_5px_0px_0px_#3b82f6]"
                >
                  ...
                  {/* Desna strana: Confirmed + Action Buttons */}
                  <div className="flex flex-col items-center gap-3 shrink-0 ml-4">
                    ...
                    <div className="flex flex-col gap-2">
                      {job.status === "in_progress" && (
                        <button
                          onClick={() => handleCompleteVisit(job.id)}
                          disabled={actionLoadingId === job.id}
                          className="group flex items-center justify-center gap-2 border-[3px] border-black bg-[#EF9D39] px-4 py-2 font-black uppercase text-[10px] tracking-widest shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:bg-orange-400 hover:shadow-none rounded-[20px] disabled:opacity-50"
                        >
                      ...
                      <Link
                        href={`/${username}/dashboard/${job.id}`}
                        className="group flex items-center justify-center gap-2 border-[3px] border-black bg-white px-4 py-2 font-black uppercase text-[10px] tracking-widest shadow-[4px_4px_0px_0px_#3b82f6] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:bg-blue-500 hover:shadow-none rounded-[20px]"
                      >
              ...
              <p className="italic text-gray-400 font-bold uppercase text-xs">
                No active jobs yet.
              </p>
            )}
          </div>
          ...
          <div className="grid gap-3">
            {rejectedJobs.length > 0 ? (
              rejectedJobs.map((job) => (
                <div
                  key={job.id}
                  className="bg-gray-50 dark:bg-zinc-900 border-[3px] border-gray-300 p-4 rounded-2xl flex justify-between items-center border-dashed"
                >
                  <div>
                    <div className="flex items-center gap-2">
