...
export function getStatusInfo(booking: BookingDetail) {
  ...
  if (booking.status === "visit_fee_paid") {
    return {
      ...
      icon: (
        <Wallet className="text-sky-700 shrink-0" size={18} strokeWidth={3} />
      ),
    };
  }
  ...
  if (booking.status === "funds_locked") {
    return {
      ...
      helperText:
        "Funds are reserved. Complete work, then release escrow via payment step.",
      icon: (
  ...
  if (booking.status === "not_completed") {
    return {
      ...
      badgeClass: "bg-red-400 text-black",
      helperText:
        "Client reported the job is not completed. Follow-up is required.",
      icon: (
        <AlertCircle
  ...
  if (booking.negotiation_status === "awaiting_client") {
    return {
      ...
      badgeClass: "bg-purple-400 text-black",
      helperText:
        "Your offer (time & price) was sent. Waiting for the client to confirm.",
      icon: (
        <AlertCircle
          className="text-purple-400 shrink-0"
          size={18}
...
export default function HandymanRequestDetailsPage() {
  ...
  const [counterProposedTime, setCounterProposedTime] = useState<Date | null>(
    null,
  );
  ...
  const [secondsUntilUnlock, setSecondsUntilUnlock] = useState<number | null>(
    null,
  );
  ...
  const handleNegotiationExpire = async () => {
    ...
    try {
      ...
      if (res.data.result === "post_proposal_started") {
        setActionError("");
        setActionSuccess(
          "Client's requested time has passed. You can decline or send a counter offer — 3 hours left.",
        );
  ...
  useEffect(() => {
    if (
      !booking ||
      !["accepted", "funds_locked"].includes(booking.status) ||
  ...
  const handleAccept = async () => {
    ...
    if (
      !acceptDuration ||
      isNaN(Number(acceptDuration)) ||
      Number(acceptDuration) <= 0
    ) {
    ...
    try {
      ...
      const res = await api.post(
        `/api/bookings/${booking.id}/handyman-action/`,
        {
          action: "accept",
      ...
      setBooking(res.data);
      setActionSuccess(
        "Offer sent to the client. They must confirm the deal before the appointment is locked in.",
      );
      setAcceptOpen(false);
      setAcceptDuration("");
      setAcceptPrice("");
  ...
  const handleCounter = async () => {
    ...
    if (
      !counterDuration ||
      isNaN(Number(counterDuration)) ||
      Number(counterDuration) <= 0
    ) {
      setActionError("Please enter estimated exact time in minutes.");
      return;
    }
  ...
  const handleMarkDone = async () => {
    ...
    try {
      ...
      const res = await api.post(`/api/bookings/${booking.id}/complete/`, {
        action: "mark_done",
      });
      setBooking(res.data);
  ...
  const updateQuoteItem = (
    index: number,
    patch: Partial<QuoteLineItemInput>,
  ) => {
    setQuoteItems((prev) =>
  ...
  const submitItemizedQuote = async () => {
    ...
    if (!quoteProposedVisitTime) {
      setActionError(
        "Select the second-visit date and time before sending quote.",
      );
      return;
    }
    ...
    try {
      ...
      setActionSuccess(
        "Itemized quote sent to client. Waiting for their decision.",
      );
    } catch (e) {
      setActionError(getBackendErrorMessage(e));
    } finally {
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="text-xl font-bold uppercase">
                {booking.client_name || "No client assigned yet"}
              </div>
            </div>
            ...
            <div
              onClick={() => {
                if (booking?.status === "pending") setKnowsFix(!knowsFix);
              }}
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-colors ${knowsFix ? "bg-green-600 text-white border-green-700" : "bg-zinc-100 text-gray-400 border-black"}`}
                >
                  <CheckCircle
                    size={20}
                ...
                <div>
                  <p
                    className={`text-sm font-black uppercase tracking-tight ${knowsFix ? "text-green-700 dark:text-green-400" : "text-black dark:text-white"}`}
                  >
                    Skip Inspection?
                  </p>
              ...
              <div
                className={`w-12 h-6 rounded-full border-2 border-black relative transition-colors ${knowsFix ? "bg-green-500" : "bg-gray-200"}`}
              >
                <div
                  className={`absolute top-0.5 w-4 h-4 bg-white border-2 border-black rounded-full transition-all ${knowsFix ? "left-6" : "left-0.5"}`}
                />
              </div>
            </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[12px] font-black uppercase tracking-widest text-orange-600 dark:text-orange-400">
                      {getResponseDeadlineLabel(booking)}
                    </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-violet-600 dark:text-violet-400">
                        {booking.status === "funds_locked"
                          ? "Second visit unlocks in"
                          : "Job unlocks in"}
                      </p>
                      <p className="text-xl font-black text-black dark:text-white tabular-nums">
                        {formatMs(secondsUntilUnlock * 1000)}
                      </p>
                    </div>
                  </div>
                  <span className="text-[10px] bg-violet-500 text-white px-2 py-1 rounded font-black uppercase">
                    Scheduled
                  </span>
                </div>
              )}

            {/* ── IN PROGRESS / FUNDS LOCKED / HANDYMAN DONE: two columns — you vs client status ── */}
            {(booking.status === "in_progress" ||
              ...
              <div className="p-6 bg-violet-50 dark:bg-zinc-800/60 border-2 border-violet-500 rounded-xl space-y-4 shadow-[4px_4px_0px_0px_#7c3aed]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="rounded-xl border-2 border-red-500 bg-red-50 dark:bg-red-950/35 p-4 flex flex-col justify-center min-h-[148px] shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
              ...
              <div className="p-5 bg-amber-50 dark:bg-amber-950/20 border-2 border-amber-500 rounded-xl flex items-start gap-3 shadow-[4px_4px_0px_0px_#f59e0b]">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-amber-800 dark:text-amber-200/90 mt-1">
                    They pay from their profile balance. This updates
              ...
              <div className="p-6 bg-indigo-50 dark:bg-indigo-950/20 border-2 border-indigo-500 rounded-xl space-y-4 shadow-[4px_4px_0px_0px_#6366f1]">
                ...
                {booking.continue_job_confirmed !== true ? (
                  <p className="text-sm font-bold text-gray-700 dark:text-zinc-300">
                    ...
                    <span className="font-black">Continue the Job</span>.
                  </p>
                ) : (
                  <>
                    {booking.status === "visit_fee_paid" && (
                      <div className="space-y-3">
                        ...
                        {quoteItems.map((item, idx) => (
                          <div
                            key={idx}
                            className="grid grid-cols-12 gap-2 bg-white dark:bg-zinc-900 border-2 border-black rounded-xl p-3"
                          >
                            <input
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                updateQuoteItem(idx, {
                                  category: e.target
                                    .value as QuoteLineItemInput["category"],
                                })
                              }
                    ...
                    {booking.latest_quote && (
                      <div className="p-4 bg-white dark:bg-zinc-900 border-2 border-black rounded-xl">
                        ...
                        <p className="text-sm font-bold">
                          Version #{booking.latest_quote.version} -{" "}
                          {booking.latest_quote.status}
                        </p>
            ...
            {booking.status === "closed" && (
              <div className="p-6 bg-green-50 dark:bg-green-950/25 border-2 border-green-500 rounded-xl text-center shadow-[4px_4px_0px_0px_#22c55e]">
                <CheckCircle2
                  ...
                  strokeWidth={2.5}
                />
                <h3 className="font-black uppercase text-lg text-black dark:text-white">
                  Job finished
                </h3>
                <p className="text-sm font-bold text-gray-600 dark:text-zinc-400 mt-1">
                  See you on the next booking.
                </p>
              </div>
            )}
            ...
            {booking.status === "completed" && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl border-2 border-green-500 bg-green-50 dark:bg-green-950/30 p-4 flex items-center gap-3 shadow-[3px_3px_0px_0px_#16a34a]">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-green-700 dark:text-green-400">
                      Handyman
                    </p>
            ...
            {booking.status === "not_completed" && (
              <div className="p-5 bg-red-50 dark:bg-zinc-800/60 border-2 border-red-400 rounded-xl flex items-center justify-between gap-3 shadow-[4px_4px_0px_0px_#ef4444]">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black uppercase text-sm text-black dark:text-white">
                      Client marked job as not completed
                    </p>
                    <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                      Escrow was refunded. Reopen after agreement with client.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="space-y-4">
                ...
                {acceptOpen && showSendOffer && (
                  <div className="p-4 md:p-5 border-[3px] border-black rounded-2xl bg-green-50 dark:bg-zinc-900 animate-in slide-in-from-top-2 shadow-[6px_6px_0px_0px_#000] space-y-3">
                    <p className="text-xs font-black uppercase tracking-wide text-gray-700 dark:text-zinc-300">
                      Enter how long the job should take and your total price.
                      This is sent to the client to confirm — no payment happens
                      until later in the flow.
                    </p>
                    <div className="space-y-3">
                      <div>
                        <label className="text-[11px] font-black uppercase tracking-widest mb-2 block text-gray-600 dark:text-zinc-400">
                          Appointment time (from request)
                        </label>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="relative">
                            ...
                            />
                            <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-black text-xs uppercase text-gray-400">
                              min
                            </span>
                          </div>
                        </div>
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                setAcceptPrice(
                                  sanitizeKmPriceInput(e.target.value),
                                )
                              }
                            />
                            <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-black text-xs uppercase text-gray-400">
                ...
                {counterOpen && (
                  <div className="border-[3px] border-black rounded-2xl bg-[#FFF8EA] dark:bg-zinc-900 p-4 md:p-5 space-y-3 animate-in slide-in-from-top-2 shadow-[6px_6px_0px_0px_#000]">
                    <p className="text-xs font-black uppercase tracking-wide text-gray-700 dark:text-zinc-300">
                      Propose a new time, estimated exact duration, and total
                      price. The client confirms before the booking is final —
                      payment stays later.
                    </p>
                    <div className="flex flex-col gap-2">
                      <label className="text-[11px] font-black uppercase tracking-widest text-gray-600 dark:text-zinc-400">
                        Pick new appointment time *
                      </label>
                      <div
                      ...
                      >
                        ...
                        {counterProposedTime ? (
                          <span className="text-black dark:text-white">
                            {formatDateTime(counterProposedTime)}
                          </span>
                        ) : (
                          <span className="text-gray-400 text-xs uppercase">
                ...
                <div className="bg-white dark:bg-zinc-900 border-4 border-black rounded-[40px] shadow-[20px_20px_0px_0px_rgba(0,0,0,1)] p-6 md:p-10 max-w-2xl w-full relative flex flex-col items-center">
                  ...
                  <div className="flex justify-center w-full bg-white dark:bg-zinc-900 rounded-3xl border-2 border-black/10 pt-4 overflow-hidden">
                    <DatePicker
                      ...
                      onChange={(date: Date | null) =>
                        setQuoteProposedVisitTime(date)
                      }
                      inline
                      showTimeSelect
                      timeIntervals={5}
      ...
      {jobFinishedModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-white dark:bg-zinc-900 border-[3px] border-black dark:border-zinc-700 p-8 rounded-[24px] shadow-[8px_8px_0px_0px_rgba(34,197,94,0.45)] text-center">
            ...
            <p className="text-sm font-bold text-gray-600 dark:text-zinc-400 mt-2">
              Thank you for your work on GetItFixed.
            </p>
            <button
              type="button"
