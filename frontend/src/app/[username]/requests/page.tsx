...
function getRequestState(request: BookingDetail) {
  if (
    ...
    request.negotiation_status === "declined"
  ) {
    return {
      label: "Cancelled",
  ...
  if (request.status === "visit_fee_paid") {
    return {
      ...
      message:
        "Initial visit fee step is done. Continue-job decision is active.",
    };
  }
  ...
  if (request.status === "quote_pending_client") {
    return {
      ...
      message:
        "Handyman sent an itemized quote. Open details to accept or reject.",
    };
  }
  ...
  if (request.status === "handyman_done") {
    return {
      ...
      message:
        "Handyman marked the job as finished. Confirm completion in request details.",
    };
  }
  ...
  if (request.status === "not_completed") {
    return {
      ...
      badgeClass: "bg-red-400 text-black",
      message:
        "You marked this job as not completed. Follow-up with handyman is required.",
    };
  }
  ...
  if (request.status === "awaiting_payment") {
    return {
      ...
      message: "Work is confirmed. Pay from your profile balance to finish.",
    };
  }
  ...
  if (request.status === "paid") {
    return {
      ...
      message: "Payment sent. Waiting for the expert to acknowledge.",
    };
  }
  ...
  if (request.negotiation_status === "awaiting_client") {
    return {
      ...
      badgeClass: "bg-purple-400 text-black",
      message:
        "The expert sent time & price — open the ticket to confirm or deny.",
    };
  }
...
function getRequestCategory(
  request: BookingDetail,
): Exclude<RequestFilterId, "all"> {
  if (
    request.status === "cancelled" ||
    request.negotiation_status === "declined"
  ) {
  ...
  const filterCounts = REQUEST_FILTERS.reduce(
    (acc, filter) => {
      if (filter.id === "all") {
        ...
        acc[filter.id] = requests.filter(
          (r) => getRequestCategory(r) === filter.id,
        ).length;
      }
      return acc;
    },
        ...
        prev.map((req) =>
          req.id === bookingId
            ? {
                ...
                negotiation_status:
                  "declined" as BookingDetail["negotiation_status"],
              }
            : req,
        ),
      );
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <h3 className="text-md font-black uppercase tracking-tight text-zinc-400">
                            ...
                            <span className="text-black dark:text-white ">
                              {req.handyman_name
                                ? req.handyman_name
                                : "No Handyman assigned yet"}
                            </span>
                          </h3>
                        </div>

                        <span
                          className={`px-4 py-1 border-2 border-black font-black text-[10px] uppercase rounded-full shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${requestState.badgeClass}`}
                        >
                          {requestState.label}
                        </span>
                      </div>
