...
export function getBookingColorClass(booking: BookingDetail): string {
  const { status, negotiation_status } = booking;

  if (status === "cancelled" || negotiation_status === "declined")
    return "bg-red-400";

  if (status === "in_progress")
    return "bg-violet-400";

  if (status === "handyman_done")
    return "bg-purple-400";

  if (status === "awaiting_payment")
    return "bg-amber-400";

  if (status === "visit_fee_paid")
    return "bg-sky-300";
  ...
  if (status === "accepted" || negotiation_status === "agreed")
    return "bg-blue-400";

  if (negotiation_status === "awaiting_client")
    return "bg-purple-400";

  // pending / awaiting_handyman / default → yellow (Waiting)
  return "bg-yellow-400";
}

export function getBookingStatusLabel(booking: BookingDetail): string {
  const { status, negotiation_status } = booking;

  if (status === "cancelled" || negotiation_status === "declined") return "Cancelled";
  if (status === "in_progress") return "In Progress";
  if (status === "handyman_done") return "Awaiting Confirm";
  if (status === "awaiting_payment") return "Payment Due";
  if (status === "visit_fee_paid") return "Visit Paid";
  if (status === "quote_pending_client") return "Quote Review";
  if (status === "funds_locked") return "Funds Locked";
  if (status === "paid") return "Paid";
  if (status === "closed" || status === "completed") return "Completed";
  if (status === "not_completed") return "Not Completed";
  if (status === "accepted" || negotiation_status === "agreed") return "Accepted";
  if (negotiation_status === "awaiting_client") return "Expert Offer";
  return "Waiting";
}

// ─── Legend items (sve kategorije) ──────────────────────────────────────────
const STATUS_LEGEND = [
  { label: "Waiting",          color: "bg-yellow-400" },
  { label: "Expert Offer",     color: "bg-purple-400" },
  { label: "Accepted",         color: "bg-blue-400" },
  { label: "In Progress",      color: "bg-violet-400" },
  { label: "Awaiting Confirm", color: "bg-purple-400" },
  { label: "Payment Due",      color: "bg-amber-400" },
  { label: "Visit Paid",       color: "bg-sky-300" },
  { label: "Quote Review",     color: "bg-indigo-300" },
  { label: "Funds Locked",     color: "bg-cyan-300" },
            ...
            <div className="grid grid-cols-7 border-[3px] border-black rounded-2xl overflow-hidden bg-black gap-[2px] shadow-[10px_10px_0px_0px_rgba(0,0,0,0.1)]">
              {eachDayOfInterval({
                ...
                const dayBookings = bookings.filter(b =>
                  isSameDay(new Date(b.scheduled_time || b.client_proposed_time || ""), day)
                );
                ...
                return (
                  <div
                    key={idx}
                    className={`min-h-[110px] p-2 ${isCurrentMonth ? "bg-white dark:bg-zinc-800" : "bg-zinc-50 dark:bg-zinc-900 opacity-30"}`}
                  >
                    ...
                    <div className="mt-1 space-y-1">
                      {dayBookings.map(b => {
                        const colorClass = getBookingColorClass(b);
                        return (
                          <div
                            key={b.id}
                            onClick={() => setSelectedJob(b)}
                            className={`text-[9px] p-1.5 border-2 border-black font-bold rounded-lg cursor-pointer shadow-[2px_2px_0px_0px_#000] text-black hover:translate-y-0.5 hover:shadow-none transition-all ${colorClass}`}
                          >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-zinc-900 border-[4px] border-black p-8 rounded-[32px] shadow-[15px_15px_0px_0px_#000] max-w-md w-full relative animate-in zoom-in-95 duration-200">
            <div className="relative mb-8 text-center">
              ...
              <div className="absolute -top-2 -right-2 w-8 h-8 bg-[#EF9D39] rounded-full border-2 border-black -z-10 animate-pulse" />
            </div>

            <div className="space-y-4 text-left">
              {/* Status badge */}
              <div className={`flex items-center justify-center gap-2 px-4 py-2 border-2 border-black rounded-xl font-black uppercase text-sm ${getBookingColorClass(selectedJob)}`}>
                {getBookingStatusLabel(selectedJob)}
              </div>
              ...
              <div className="flex items-start gap-4 p-3 border-2 border-black rounded-xl bg-zinc-50 dark:bg-zinc-800">
                ...
                <div>
                  ...
                  <p className="font-bold dark:text-white">
                    {format(new Date(selectedJob.scheduled_time || selectedJob.client_proposed_time || ""), "PPP p")}
                  </p>
                </div>
              </div>
            ...
            <button
              ...
              className="cursor-pointer w-full mt-8 bg-black text-white py-4 rounded-2xl font-black uppercase tracking-widest shadow-[5px_5px_0px_0px_#EF9D39] active:translate-y-1 active:shadow-none transition-all"
            >
              Close
            </button>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border-[3px] border-black p-6 rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {STATUS_LEGEND.map(({ label, color }) => (
              <div
                key={label}
                className={`flex items-center gap-3 p-2 border-2 border-black rounded-xl ${color} shadow-[3px_3px_0px_0px_#000]`}
              >
