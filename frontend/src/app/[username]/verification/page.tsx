...
export default function AdminVerificationPage() {
  ...
  const handleAction = async (
    ...
    try {
      ...
      // Instant local state synchronization
      setProviders((prev) =>
        prev.map((p) =>
          p.id === userId ? { ...p, is_active: actionType === "approve" } : p,
        ),
      );
    } catch (err) {
      ...
      <main className="flex-grow max-w-5xl mx-auto p-6 py-12 w-full">
        <div className="mb-8">
          ...
          <p className="text-gray-500 font-medium text-sm mt-1">
            Audit onboarding requests and manage platform access privileges.
          </p>
        </div>
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-md border border-black ${provider.is_active ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                    >
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  {provider.is_active ? (
                    <button
                      ...
                      className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 border-2 border-black bg-red-500 hover:bg-red-600 text-white font-black uppercase text-xs rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-transform active:translate-x-0.5 active:translate-y-0.5"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 border-2 border-black bg-green-500 hover:bg-green-600 text-white font-black uppercase text-xs rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-transform active:translate-x-0.5 active:translate-y-0.5"
                    >
                      <UserCheck size={14} /> Approve Provider
                    </button>
                  )}
                </div>
              </div>
