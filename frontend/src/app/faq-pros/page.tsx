...
export default function ProviderFaqPage() {
  ...
  const faqs = [
    { q: "How do I join as a provider?", a: <>Click <Link href="/register?role=pro"><span className="underline font-black text-black dark:text-[#f0f0f0] hover:text-[#EF9D39] transition-colors">Join as a Pro</span></Link> and complete your profile — add your specialties and contact info.</> },
    ...
    <div className="page-gradient flex flex-col min-h-screen font-sans text-black dark:text-white">
      ...
      <main className="flex-grow max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <div className="space-y-5">
          {faqs.map((f, i) => (
            <div key={i} className="bg-white dark:bg-[#141414] border-[3px] border-black dark:border-[#222] p-6 rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-none">
              <h3 className="font-black uppercase text-lg mb-3 tracking-tight text-black dark:text-[#f0f0f0]">{f.q}</h3>
              <div className="text-[13px] font-medium text-gray-600 dark:text-[#888] leading-relaxed">
                {f.a}
              </div>
            </div>
          ))}
        ...
        <div className="mt-12 flex flex-col items-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 dark:text-[#555] text-center">
